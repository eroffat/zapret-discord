...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$avOn=$false; try { $d=Get-MpComputerStatus -EA SilentlyContinue; if ($d.RealTimeProtectionEnabled) { $avOn=$true } } catch {}; if ($avOn) { Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Windows Defender включен. Отключите защиту в реальном времени и запустите general (ALT9).bat снова.','general (ALT9).bat — запуск отменён',[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null; exit 1 } else { exit 0 }"
